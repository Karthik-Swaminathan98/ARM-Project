...
void benchmark_arm_cfft_q15(arm_cfft_instance_q15* fft_instance, q15_t* input, uint32_t clkFastfreq,
...
void benchmark_arm_cfft_q15(arm_cfft_instance_q15* fft_instance, q15_t* input, uint32_t clkFastfreq,
    ...
    arm_cfft_q15(fft_instance, input, 0, 1);  // Forward FFT with bit reversal
...
RAM_FUNC int main(void) {
    ...
    for (int size_idx = 0; size_idx < FFT_SIZES_COUNT; size_idx++) {
        ...
        for (int execution = 0; execution < NUM_EXECUTIONS; execution++) {
            ...
            benchmark_arm_cfft_q15(&fft_instance, input, clkFastfreq,
